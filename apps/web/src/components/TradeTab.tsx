...
export function TradeTab() {
  ...
  return <main className="mx-auto max-w-5xl space-y-5 px-4 pb-10 pt-6 sm:px-6">
    ...
    {view === 'market' ? <>
      ...
      {portfolio.positions.map(position => <article key={position.id} className="rounded-2xl border border-white/10 bg-slate-900 p-5"><div className="flex flex-wrap justify-between gap-3"><div><h3 className="font-semibold">{position.asset.symbol}</h3><p className="mt-1 text-xs text-slate-400">{position.asset.name} · {position.asset.exchange}</p></div><div className="text-right"><p>{money(position.currentValueEur)}</p><p className={`text-xs ${position.profitLossEur >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>{money(position.profitLossEur)} · {percent(position.profitLossPercent)}</p></div></div><p className="mt-3 text-xs text-slate-300">{decimal(position.units)} unidades · Invertido: {money(position.investedEur)} · Precio medio: {money(position.averageBuyPrice, position.asset.currency)}</p><p className="mt-2 text-xs text-slate-500">{position.source === 'illustrative' ? 'Valoración ilustrativa' : 'Twelve Data'}{position.stale ? ' · Sin actualizar' : ''} · {new Date(position.asOf).toLocaleString('es-ES')}</p><button onClick={() => choose(position.asset)} className="mt-4 text-xs text-emerald-200">Revisar y ampliar posición</button></article>)}
    </section>}
