...
const goalIcons: Record<InvestorGoal, KaiIconName> = {
  'Aprender a invertir': 'learn', 'Gestionar mi dinero': 'growth', 'Crear mi colchón': 'shield',
  'Invertir a largo plazo': 'globe', 'Seguir inversores': 'network',
};
const headings = ['¿Qué quieres conseguir?', 'Visibilidad del perfil', 'Los últimos detalles'];
const dialogue = ['Configuremos tu espacio. ¿Hacia dónde apuntamos?', 'Elige la visibilidad para pasar a tus datos. Privado: solo tú puedes consultar tu perfil. Público: otras personas podrán consultarlo y seguirte tras la verificación simulada.', 'Casi listos. Completemos tus datos y comprobemos la edad de acceso a esta demo.'];
  ...
  return <div className="mx-auto flex min-h-[calc(100dvh-6rem)] w-full max-w-md flex-col">
    ...
    <form onSubmit={event => void submit(event)} className="flex flex-1 flex-col">
      ...
      <div className="sticky bottom-0 z-10 flex justify-between gap-3 border-t border-white/5 bg-slate-950/95 py-4 backdrop-blur-md">
        ...
        {currentStep === 3 && <button type="submit" disabled={busy} className="rounded-full bg-gradient-to-r from-cyan-300 to-emerald-300 px-6 py-3 text-sm font-semibold text-slate-950 shadow-[0_0_20px_rgba(52,211,153,0.2)] transition disabled:opacity-35 disabled:shadow-none">{busy ? 'Guardando…' : 'Continuar'} <span aria-hidden="true">→</span></button>}
      </div>
    </form>
  </div>;
}
