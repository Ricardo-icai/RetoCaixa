...
export default function VerifyIdentity() {
  ...
  useEffect(() => {
    fetch('/api/community', { cache: 'no-store' }).then(async response => {
      ...
      setNationality(current.onboarding.nationality ?? '');
  ...
  async function submitProfile(event: React.FormEvent) {
    ...
    const next = await act({ operation: 'completeOnboarding', goals, visibility, countryCode, nationality, acceptTerms: terms, acknowledgePrivacy: privacy, acceptRisk: risk, dateOfBirth: birthDateToISO(dateOfBirth), legalVersions });
    ...
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <section className="rounded-3xl border border-white/10 bg-[#101d2a] p-6 sm:p-8">
          ...
          {snapshot && step === 'profile' && <form onSubmit={submitProfile} className="space-y-6"><div><h2 className="text-2xl font-semibold">¿Qué quieres conseguir?</h2><p className="mt-2 text-sm text-slate-400">Elige una o varias opciones.</p></div><div className="grid gap-2 sm:grid-cols-2">{investorGoals.map(goal => <button key={goal} type="button" onClick={() => toggleGoal(goal)} aria-pressed={goals.includes(goal)} className={`rounded-xl border p-4 text-left text-sm ${goals.includes(goal) ? 'border-emerald-300 bg-emerald-300/10 text-emerald-100' : 'border-white/10 text-slate-300 hover:border-white/30'}`}>{goal}</button>)}</div><div className="grid gap-4 sm:grid-cols-2"><label className="text-xs text-slate-300">País de residencia<select value={countryCode} onChange={event => setCountryCode(event.target.value)} className="mt-2 w-full rounded-xl border border-white/15 bg-[#08111b] p-3 text-sm"><option value="ES">España</option><option value="MX">México</option><option value="US">Estados Unidos</option><option value="ZZ">Otro</option></select></label><label className="text-xs text-slate-300" htmlFor="nationality">Nacionalidad<input id="nationality" name="nationality" required minLength={2} maxLength={80} value={nationality} onChange={event => setNationality(event.target.value)} placeholder="Ej.: española, mexicana…" aria-describedby="nationality-help" className="mt-2 w-full rounded-xl border border-white/15 bg-[#08111b] p-3 text-sm" /><span id="nationality-help" className="mt-2 block leading-5 text-slate-400">Puede ser distinta de tu país de residencia. Si tienes varias, sepáralas con comas. No se muestra en tu perfil público.</span></label><fieldset><legend className="text-xs text-slate-300">Visibilidad del perfil</legend><div className="mt-2 flex gap-2">{(['PRIVATE', 'PUBLIC'] as const).map(item => <button key={item} type="button" onClick={() => setVisibility(item)} aria-pressed={visibility === item} className={`flex-1 rounded-xl border p-3 text-xs ${visibility === item ? 'border-emerald-300 text-emerald-200' : 'border-white/15 text-slate-400'}`}>{item === 'PRIVATE' ? 'Privado' : 'Público'}</button>)}</div></fieldset></div><BirthDateInput value={dateOfBirth} onChange={setDateOfBirth} /><div className="space-y-3 border-t border-white/10 pt-5"><p className="text-xs leading-6 text-slate-400">Demo disponible desde los 18 años; todavía sin cuentas tuteladas. La verificación es simulada; no se capturan documentos ni biometría. <a href="/legal" target="_blank" rel="noopener noreferrer" className="text-emerald-200 underline">Consultar el centro legal (nueva pestaña)</a>.</p>
...
</div><button disabled={busy || goals.length === 0 || !terms || !privacy || !risk || !birthDateToISO(dateOfBirth) || nationality.trim().length < 2} className="w-full rounded-xl bg-emerald-300 p-4 text-sm font-semibold text-slate-950 disabled:opacity-40">Continuar ↗</button></form>}
