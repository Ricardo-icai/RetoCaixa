...
import { ReelVideoFeed } from '../components/ReelVideoFeed';
...
export default function Community() {
  const [view, setView] = useState<'reels' | 'posts'>('reels');
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (version === requestVersion.current) setError(cause instanceof Error ? cause.message : 'No se ha podido abrir la comunidad.');
    }
  }, []);

  useEffect(() => {
    const refresh = () => { if (!document.hidden && !mutating.current) void load(); };
    refresh();
  ...
  useEffect(() => {
    ...
    if (!snapshot || !hash.startsWith('#post-') || openedPost.current === hash) return;
    if (view !== 'posts') { setView('posts'); return; }
    const element = document.getElementById(hash.slice(1));
    if (element) {
      element.scrollIntoView({ behavior: 'smooth' });
      openedPost.current = hash;
    } else {
      const post = snapshot.posts.find(item => `#post-${item.id}` === hash);
      if (post) setFilter(post.topic);
    }
  }, [snapshot, filter, view]);

  async function act(body: Record<string, unknown>, success: string) {
    if (!snapshot || mutating.current) return false;
  ...
  const posts = useMemo(() => {
    if (!snapshot) return [];
    // Keep a balanced selection in Para ti; other tabs retain the whole catalogue.
    if (filter === 'Para ti') return snapshot.posts.slice(0, snapshot.feed.preview.total);
    return snapshot.posts.filter(post => filter === 'Siguiendo' ? snapshot.channels.some(channel => channel.id === post.channelId && channel.subscribed) : post.topic === filter);
  }, [snapshot, filter]);
  ...
  return <div className="min-h-screen bg-[#0a111b] text-slate-100">
    ...
    <header className="border-b border-white/10 bg-[#0d1723]/95 px-5 py-4">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3"><KaiHeaderIcon /><div className="flex rounded-xl border border-white/10 p-1" aria-label="Formato del feed">{(['reels', 'posts'] as const).map(item => <button key={item} aria-pressed={view === item} onClick={() => setView(item)} className={`rounded-lg px-4 py-2 text-xs font-semibold ${view === item ? 'bg-emerald-300 text-slate-950' : 'text-slate-300'}`}>{item === 'reels' ? 'Reels' : 'Comunidad'}</button>)}</div></div>
    </header>

    {view === 'reels' ? <main className="px-3 pb-4 pt-4">{snapshot ? <ReelVideoFeed snapshot={snapshot} onRefresh={() => void load()} /> : <p className="p-8 text-center text-sm text-slate-400">{error || 'Cargando reels…'}</p>}</main> : <main className="mx-auto max-w-7xl px-4 pb-16 pt-7 sm:px-6">
      ...
      <div className="mt-7 grid grid-cols-1 items-start gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="min-w-0 space-y-6">
          ...
          <section aria-labelledby="feed-title">
            ...
            {snapshot && filter === 'Para ti' && (snapshot.feed.mode === 'personalized' ? <details className="mb-5 rounded-2xl border border-emerald-300/20 bg-emerald-300/5 px-4 py-3 text-xs text-slate-300">
              <summary className="cursor-pointer font-medium text-emerald-200">Tu mezcla 70/30 · {snapshot.feed.priorityTopics.join(' · ')}</summary>
              <p className="mt-3 leading-6">Buscamos un 70 % relacionado con tus objetivos y lo que necesitas aprender, y un 30 % de otros temas para ampliar tu perspectiva. {snapshot.feed.source === 'onboarding' ? 'Por ahora usamos los objetivos de tu perfil.' : snapshot.feed.source === 'saved' ? 'KAI se guía por tus conversaciones guardadas.' : 'KAI adapta la selección a lo que habláis.'}</p>
              <p className="mt-2 leading-6">La proporción depende del contenido disponible. Ahora, entre las primeras {snapshot.feed.preview.total} publicaciones: {snapshot.feed.preview.personalized} para ti y {snapshot.feed.preview.discovery} de descubrimiento.</p>
              <p className="mt-2 leading-6">Resetear el chat elimina su influencia. Las conversaciones guardadas siguen contando hasta que las elimines.</p>
            </details> : <p className="mb-5 text-xs leading-6 text-slate-400">Habla con KAI para adaptar el feed a tus objetivos. Mientras tanto, descubre temas variados.</p>)}
            ...
            <div className="space-y-4">{posts.map(post => <article id={`post-${post.id}`} key={post.id} className="rounded-3xl border border-white/10 bg-[#101d2a] p-5 sm:p-6">
              ...
              <div className="mt-5 flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-medium text-emerald-200">{post.topic}</p>{filter === 'Para ti' && post.recommendation && <details className="text-xs text-slate-400"><summary className="cursor-pointer">{post.recommendation.kind === 'personalized' ? 'Para tu objetivo' : 'Descubrimiento'}</summary><p className="mt-2 max-w-xs leading-5">{post.recommendation.reason}</p></details>}</div><h3 className="mt-2 text-xl font-semibold leading-7">{post.title}</h3><p className="mt-3 whitespace-pre-wrap break-words text-sm leading-7 text-slate-300">{post.text}</p>
