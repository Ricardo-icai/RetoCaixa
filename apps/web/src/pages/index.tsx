...
export default function Home() {
  ...
  const handleAction = () => {
    if (copilotResponse.recommendedAction === 'INVEST') {
      if (!window.confirm(`¿Confirmas la inversión simulada de €${userProfile.availableInvestableCash}? Puedes perder parte del capital en una inversión real. Coste anual ilustrativo: 0,20 %. No se moverá dinero real.`)) return;
      setSimulatedBalance(value => value + userProfile.availableInvestableCash);
  ...
  const handleSimulateProtect = () => {
    const updated = { ...userProfile, emergencyBufferMonths: 1 };
    setUserProfile(updated);
    setCopilotResponse(InvestmentDecisionEngine.evaluateUserSnapshot(updated));
  };

  const handleSimulateNormal = () => {
    const updated = { ...userProfile, emergencyBufferMonths: 4 };
    setUserProfile(updated);
    setCopilotResponse(InvestmentDecisionEngine.evaluateUserSnapshot(updated));
  };
