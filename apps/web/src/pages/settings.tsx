...
export default function Settings() {
  ...
  return <main className="mx-auto max-w-3xl space-y-7 px-5 py-10 text-slate-100">
    ...
    <section className="rounded-2xl border border-white/10 p-6"><h2 className="text-lg font-semibold">Tu aceptación</h2>
      {error ? <p role="alert" className="mt-3 text-sm text-rose-200">{error}</p> : !snapshot ? <p className="mt-3 text-sm text-slate-400">Consultando…</p> : receipt ? <div className="mt-3 space-y-3 text-sm text-slate-300"><p>{currentLegalAcceptance(receipt) ? 'Versiones vigentes aceptadas' : 'Hay versiones pendientes de aceptar'}</p><p>Fecha: {new Date(receipt.acceptedAt).toLocaleString('es-ES')}</p><p>Términos: {receipt.versions.terms} · Privacidad: {receipt.versions.privacy} · Riesgos: {receipt.versions.risk}</p><p>País declarado: {receipt.countryCode}. Mayoría de edad declarada.</p><p className="text-xs text-slate-400">Registro temporal de esta demo. Puede perderse al reiniciar el servidor.</p></div> : <p className="mt-3 text-sm text-slate-300">No hay una aceptación registrada en esta sesión.</p>}
