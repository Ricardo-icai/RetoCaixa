    ...
    <main className="mx-auto max-w-5xl px-4 pb-16 pt-8 sm:px-6">
      ...
      {channel && <>
        <section className="rounded-3xl border border-emerald-300/20 bg-gradient-to-br from-[#173b3a] via-[#142c36] to-[#132033] p-6 sm:p-9">
          <div className="flex flex-wrap items-start gap-5"><div aria-hidden="true" className="grid h-20 w-20 shrink-0 place-items-center rounded-3xl bg-emerald-300/20 text-4xl font-semibold text-emerald-200">{channel.name.charAt(0)}</div><div className="min-w-0 flex-1"><h1 className="text-3xl font-semibold sm:text-4xl">{channel.name}</h1><p className="mt-2 text-sm text-slate-200">{channel.focus}</p></div><FollowButton userId={channel.id} following={channel.subscribed} csrfToken={snapshot!.csrfToken} onChanged={() => { fetch('/api/community', { cache: 'no-store' }).then(async response => { if (response.ok) setSnapshot(await response.json()); }).catch(() => setError('No se ha podido actualizar el canal.')); }} /></div>
          <p className="mt-6 max-w-2xl text-sm leading-7 text-slate-200">{channel.bio}</p><p className="mt-3 text-xs text-slate-400">{channel.subscriberCount} seguidores · Canal gratuito</p>
        </section>
