...
export default function ChannelPage() {
  ...
  return <div className="min-h-screen bg-[#0a111b] text-slate-100">
    ...
    <header className="border-b border-white/10 px-5 py-4"><div className="mx-auto flex max-w-5xl items-center justify-between gap-4"><a href="/community" className="text-xl font-semibold">imagin <span className="text-emerald-300">/ KAI</span></a><a href="/community" className="rounded-full border border-white/15 px-4 py-2 text-xs text-slate-300 hover:border-emerald-300">← Volver a la comunidad</a></div></header>
    <main className="mx-auto max-w-5xl px-4 pb-16 pt-8 sm:px-6">
      ...
      {snapshot && !channel && <p className="rounded-2xl border border-white/10 p-8 text-sm text-slate-300">Este canal no existe. <a href="/community" className="text-emerald-200 underline">Explora la comunidad</a>.</p>}
      {channel && <>
        <section className="rounded-3xl border border-emerald-300/20 bg-gradient-to-br from-[#173b3a] via-[#142c36] to-[#132033] p-6 sm:p-9">
          <div className="flex flex-wrap items-start gap-5"><div aria-hidden="true" className="grid h-20 w-20 shrink-0 place-items-center rounded-3xl bg-emerald-300/20 text-4xl font-semibold text-emerald-200">{channel.name.charAt(0)}</div><div className="min-w-0 flex-1"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-200">Canal ficticio · Demo</p><h1 className="mt-2 text-3xl font-semibold sm:text-4xl">{channel.name}</h1><p className="mt-2 text-sm text-slate-200">{channel.focus}</p></div><button disabled={busy} onClick={() => void toggleFollow()} aria-pressed={channel.subscribed} className={`rounded-xl px-5 py-3 text-sm font-semibold disabled:opacity-40 ${channel.subscribed ? 'border border-emerald-300/40 text-emerald-200' : 'bg-emerald-300 text-slate-950'}`}>{channel.subscribed ? 'Siguiendo' : 'Seguir gratis'}</button></div>
          <p className="mt-6 max-w-2xl text-sm leading-7 text-slate-200">{channel.bio}</p><p className="mt-4 text-xs text-slate-400">{channel.subscriberCount} siguiendo en esta demo · Todas las publicaciones son gratuitas</p>
        </section>
        <p className="mt-5 rounded-xl border border-amber-300/15 bg-amber-300/5 p-4 text-xs leading-6 text-amber-100">Este personaje es ficticio. Los movimientos son simulados y los consejos son educativos; no representan operaciones de una persona real ni recomendaciones personalizadas.</p>
