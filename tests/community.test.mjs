...
function verify(viewer, goals = ['Aprender a invertir']) {
  mutateCommunity(viewer, { operation: 'completeOnboarding', goals, visibility: 'PRIVATE', countryCode: 'ES', nationality: 'Española', acceptTerms: true, legalVersions, dateOfBirth: '1990-05-15', acknowledgePrivacy: true, acceptRisk: true });
...
test('onboarding requires legal consent and verification before publishing', () => {
  ...
  assert.throws(() => mutateCommunity(viewer, { operation: 'completeOnboarding', goals: ['Gestionar mi dinero'], visibility: 'PRIVATE', countryCode: 'ES', nationality: 'Española', acceptTerms: true, legalVersions, dateOfBirth: '1990-05-15', acknowledgePrivacy: false, acceptRisk: true }), error => error.status === 400);
  mutateCommunity(viewer, { operation: 'completeOnboarding', goals: ['Gestionar mi dinero'], visibility: 'PUBLIC', countryCode: 'ES', nationality: 'Española', acceptTerms: true, legalVersions, dateOfBirth: '1990-05-15', acknowledgePrivacy: true, acceptRisk: true });
